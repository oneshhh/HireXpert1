...
app.get("/api/candidates/with-status", async (req, res) => {
  try {
    const { department } = req.query;

    // 1) Get all candidates from DB_A
    const q = `
      SELECT 
        cs.session_id,
        cs.interview_id,
      ...
      FROM candidate_sessions cs
      JOIN interviews i ON cs.interview_id = i.id
      ${department ? "WHERE cs.department = $1" : ""}
      ORDER BY cs.created_at DESC
    `;

    const params = department ? [department] : [];
    const result = await pool.query(q, params);
    const candidatesA = result.rows;

    // 2) Get all answers from DB_B (Supabase)
    const answersRes = await supabase_second_db
      .from("answers")
      .select("candidate_token, candidate_email, interview_id, status");

    if (answersRes.error) {
      console.error("Supabase answers fetch error:", answersRes.error);
      return res.status(500).json({ error: "ANSWERS_DB_ERROR" });
    }

    const answersB = answersRes.data || [];

    // GROUP ANSWERS BY candidate_email + interview_id
    const grouped = {};

    for (const row of answersB) {
      const email = row.candidate_email?.toLowerCase();
      const iid = row.interview_id;

      if (!email || !iid) continue;

      const key = `${email}|${iid}`;

      if (!grouped[key]) {
        grouped[key] = { total: 0, ready: 0, candidate_token: row.candidate_token };
      }

      grouped[key].total++;
      if (row.status === "ready") grouped[key].ready++;
    }

    // 3) Build final candidate list (submitted/pending)
    const finalList = [];

    for (const c of candidatesA) {
      const email = c.candidate_email.toLowerCase();
      const key = `${email}|${c.interview_id}`;
      const ans = grouped[key] || null;

      let status = "Pending";
      let candidateToken = null;

      if (ans && ans.total > 0 && ans.ready === ans.total) {
        status = "Submitted";
        candidateToken = ans.candidate_token;
      }
